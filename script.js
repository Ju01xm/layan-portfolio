...
// ==========================================
// 2. دالة عرض البيانات في الواجهة (للتسريع)
// ==========================================
function renderSiteContent(data) {
    ...
    if (data.hero) {
        ...
        // تحديث زر "Get in Touch"
        if (data.hero.btnText && data.hero.btnText.trim() !== "") {
            const heroBtn = document.querySelector('.dummy-btn');
            if (heroBtn) heroBtn.textContent = data.hero.btnText;
        }
    }
...
// ==========================================
// 3. جلب البيانات من Firebase وتخزينها محلياً
// ==========================================
async function loadSiteData() {
    try {
        ...
        if (docSnap.exists()) {
            ...
            // 💡 تحديث البيانات في الـ Local Storage للمرات القادمة
            localStorage.setItem('site_data_' + currentLang, JSON.stringify(data));
...
// ==========================================
// 4. جلب آخر المقالات للمدونة
// ==========================================
async function loadBlogPreviews() {
    const blogGrid = document.getElementById('blogPreviewGrid');
    if (!blogGrid) return;
    
    // إذا لم يكن هناك مقالات مخزنة محلياً، أظهر رسالة التحميل
    if(blogGrid.innerHTML.trim() === "") {
...
// ==========================================
// 6. تشغيل الأكواد الأساسية والإعداد الأولي
// ==========================================
document.addEventListener('DOMContentLoaded', async () => {
    
    applyTranslations();

    // 💡 1. عرض البيانات فوراً من التخزين المحلي (إن وجدت) لإلغاء وقت الانتظار
    const cachedData = localStorage.getItem('site_data_' + currentLang);
    if (cachedData) {
        try {
            renderSiteContent(JSON.parse(cachedData));
        } catch (e) {
            console.error("Cache format error", e);
        }
    }

    // 💡 2. جلب البيانات الأحدث من فايربيس (ستقوم بتحديث الواجهة بصمت إذا كان هناك تغيير)
    await loadSiteData();
    
    // 3. جلب مقالات المدونة
    await loadBlogPreviews();

    // 💡 4. إخفاء شاشة التحميل (Preloader) بمجرد انتهاء التحميل
